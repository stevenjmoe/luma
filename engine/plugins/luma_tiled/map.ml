...
let from_source (source : source) tilesets =
        ...
        Error
          (Luma__core.Error.io_finalize path
             (Printf.sprintf
                ...
                other))
  in
  let* stagger_axis = match stagger_axis with Some "x" -> Ok X | _ -> Ok Y in
  let* stagger_index = match stagger_index with Some "even" -> Ok Even | _ -> Ok Odd in
  ...
  let* layers =
    ...
    | other ->
        let s = Yojson.Safe.pretty_to_string other in
        Error
          (Luma__core.Error.io_finalize path (Printf.sprintf "Expected a list of layers. Got %s" s))
  in
...
(*module Format = struct
  ...
  let pp_orientation fmt = function
    | Hexagonal -> F.string fmt "Hexagonal"
    | Isometric -> F.string fmt "Isometric"
    | Orthogonal -> F.string fmt "Orthogonal"
    | Staggered -> F.string fmt "Staggered"

  let pp_axis fmt = function X -> F.string fmt "X" | Types.Y -> F.string fmt "Y"
  let pp_index fmt = function Even -> F.string fmt "Even" | Odd -> F.string fmt "Odd"
  let pp_colour fmt (_ : colour) = F.string fmt "<colour>"

  let pp_image fmt (i : image) =
  ...
  let pp fmt (m : t) =
    F.pf fmt "@[<v>%a@,%a@,%a@,%a@,%a@,%a@,%a@,%a@,%a@,%a@,%a@,%a@]"
      ...
      (kv "orientation" pp_orientation) m.orientation (kv "width" F.int) m.width (kv "height" F.int)
      m.height (kv "tile_width" F.int) m.tile_width (kv "tile_height" F.int) m.tile_height
